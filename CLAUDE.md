...
- **Fluxo de análise resiliente a contrato real (jul/2026)**: IA extrai sobre o **texto integral em janelas sobrepostas** (antes cortava em 14k chars; `openrouter.max-chunks`/`openrouter.sobreposicao-chars` em `ParametrosSistema`, merge por maior confiança, progresso "parte X de Y"). Front com **conferência guiada**: pós-upload alerta os essenciais faltantes (financiado/líquido + prazo + parcela), destaca os campos em vermelho e o `Analisar` só exige esses 3 (taxa sai por bisseção; o resto vira ressalva/inconsistência no laudo).
- **Conferência com escolha do operador (jul/2026)**: a extração **não aplica mais nada automaticamente**. IA e regex viram `candidatosExtracao` no JSONB do contrato (máx. 1 candidato por origem/campo; valores iguais colapsam na maior confiança; upload em lote acumula). Front abre SweetAlert com checkbox por candidato (exclusivo por campo, melhor confiança pré-marcada, aviso se o campo já está preenchido); confirmação → `POST /api/casos/{id}/extracao/aplicar` `[{campo, origem}]` (sobrescreve, registra `EXTRACTION_APPLY`). Botão "Conferir campos extraídos" reabre o diálogo. `PUT /casos/{id}` e `analisar` **preservam** camposExtraidos/candidatos/hashes que o form não envia (`preservarExtracao` — antes cada save apagava a trilha).
